...
class Peer(Thread):
	...
	def __init__(self, ip_addr='127.0.0.1', port=8004, sim=False, community_ip='127.0.0.1', community_port=5000, username=None, password=None):
		try:
			os.makedirs('keys')
		except OSError as e:
		...
		try:
			with open("keys/privkey4.txt","r") as fpriv:
				key = HexEncoder.decode(fpriv.read())
				self.privkey = SigningKey(key)

			with open("keys/pubkey4.txt","r") as fpub:
			...
			with open("keys/pubkey4.txt","w") as fpub:
			...
			with open("keys/privkey4.txt","w") as fpriv:
